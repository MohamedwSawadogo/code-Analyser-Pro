...
from classes_performances import DataHandler, ReferenceHandler, PerformanceAnalysis, DataVisualizer, InputSelectionDialog, OutputSelectionDialog
import pandas as pd
...
class OutilAnalyse(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Outil d'analyse des performances moteur")
        ...
        self.bouton_analyser = QPushButton("Analyser les performances", self)
        self.bouton_analyser.setGeometry(50, 200, 200, 40)
        self.bouton_analyser.clicked.connect(self.analyze_performance)
        ...
        self.table_widget.setHorizontalHeaderLabels(["Temps", "Paramètre", "Valeur", "Écart (%)", "Commentaire", "Correction"])
    ...
    def analyze_performance(self):
        if self.input_column and self.output_columns:
            analysis = PerformanceAnalysis(self.data_handler.data, self.input_column, self.output_columns, self.reference_handler)
    ...
    def populate_table(self, anomalies):
        ...
        for i, anomaly in enumerate(anomalies):
            ...
            self.table_widget.setItem(i, 1, QTableWidgetItem(anomaly['Paramètre']))
    ...
    def save_results_to_excel(self, file_path):
        ...
        df = pd.DataFrame(data, columns=["Temps", "Paramètre", "Valeur", "Écart (%)", "Commentaire", "Correction"])
