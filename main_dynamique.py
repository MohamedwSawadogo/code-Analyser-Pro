...
from classes_dynamique import DataHandler, ReferenceHandler, DynamicAnalysis, DataVisualizer, InputSelectionDialog, OutputSelectionDialog
import pandas as pd
...
class OutilAnalyse(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Outil d'analyse de la dynamique du véhicule")
        ...
        self.bouton_analyser = QPushButton("Analyser les dynamiques", self)
        self.bouton_analyser.setGeometry(50, 200, 200, 40)
        self.bouton_analyser.clicked.connect(self.analyze_dynamics)
    ...
    def analyze_dynamics(self):
        if self.input_column and self.output_columns:
            analysis = DynamicAnalysis(self.data_handler.data, self.input_column, self.output_columns, self.reference_handler)
